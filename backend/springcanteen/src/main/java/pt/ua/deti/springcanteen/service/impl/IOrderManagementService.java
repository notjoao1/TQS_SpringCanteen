...
public class IOrderManagementService implements OrderManagementService {
  ...
  public boolean manageIdleOrder(Order order) {
    ...
    if (oldOrderQueue.contains(oldOrderEntry)) {
      ...
      if (result) {
        ...
        orderRepository.save(order);
        orderNotifierService.sendOrderStatusUpdates(order.getId(), OrderStatus.PREPARING, order.isPriority());
      }
    } else {
  ...
  public boolean managePreparingOrder(Order order) {
    ...
    if (oldOrderQueue.contains(oldOrderEntry)) {
      ...
      if (result) {
        ...
        orderRepository.save(order);
        orderNotifierService.sendOrderStatusUpdates(order.getId(), OrderStatus.READY, order.isPriority());
      }
    } else {
      logger.info("Order not found in queue {}", order);
    }
  ...
  public boolean manageReadyOrder(Order order) {
    ...
    if (result) {
      ...
      orderRepository.save(order);
      orderNotifierService.sendOrderStatusUpdates(order.getId(), OrderStatus.PICKED_UP, order.isPriority());
    }
    return result;
  }
