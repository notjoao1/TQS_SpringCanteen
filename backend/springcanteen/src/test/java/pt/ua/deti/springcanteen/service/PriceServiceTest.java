...
class PriceServiceTest {
  ...
  void whenGetMenuPrice_thenPriceIsCorrectlyCalculated() {
    // setup an order for a pancakes menu with the 'Orange Juice' drink (2€) and 'Fat Pancakes' main
    // dish
    ...
    requestedOrderMenu =
        new OrderMenu(
            null,
            availableMenu,
            "{customizedDrink: {itemId: 2}, customizedMainDish: {itemId:1, customizedIngredients:"
                + " [{ingredientId: 1, quantity: 2}, {ingredientId: 2, quantity: 1}]}}");
  ...
  void whenMenu_hasInvalidMainDish_thenThrowError() {
    ...
    requestedOrderMenu =
        new OrderMenu(
            null,
            availableMenu,
            "{customizedDrink: {itemId: 2}, customizedMainDish: {itemId:3, customizedIngredients:"
    ...
    assertThat(
        ...
        containsString("Provided main dish does not exist as an option in the given menu."));
  }
  ...
  void whenMenu_hasInvalidDrink_thenThrowError() {
    ...
    requestedOrderMenu =
        new OrderMenu(
            null,
            availableMenu,
            "{customizedDrink: {itemId: 3}, customizedMainDish: {itemId:1, customizedIngredients:"
                + " [{ingredientId: 1, quantity: 2}, {ingredientId: 2, quantity: 1}]}}");
    ...
    assertThat(
        exceptionThrown.getMessage(),
        containsString("Provided drink does not exist as an option in the given menu."));
  }
  ...
  void whenMenu_isMissingIngredientsForMainDish_thenThrowError() {
    ...
    String expectedMessage =
        String.format(
            "You must specify all ingredients in the main dish and their quantity when ordering a"
                + " certain menu. Ingredient %d '%s' not found.",
            ingredient2.getId(), ingredient2.getName());
  ...
  void whenMenu_hasLessIngredientQuantityThanBase_thenPriceShouldNotBeLowerThanBasePrice() {
    ...
    // this order request removed all 'sugar' ingredient and the price should not be lowered when
    ...
    requestedOrderMenu =
        new OrderMenu(
            ...
            availableMenu,
            "{customizedDrink: {itemId: 2}, customizedMainDish: {itemId:1, customizedIngredients:"
                + " [{ingredientId: 1, quantity: 1}, {ingredientId: 2, quantity: 1}]}}");
  ...
  void whenMenu_hasMoreIngredientsThanBase_thenPriceShouldBeHigher() {
    // setup an order for a pancakes menu with the 'Water' drink (1.5€) and 'Thin Pancakes' main
    // dish
    ...
    requestedOrderMenu =
        new OrderMenu(
            null,
            availableMenu,
            "{customizedDrink: {itemId: 1}, customizedMainDish: {itemId:2, customizedIngredients:"
                + " [{ingredientId: 3, quantity: 10}, {ingredientId: 4, quantity: 4}]}}");

    float actualOrderPrice = priceService.getOrderMenuPrice(requestedOrderMenu);

    // price = drink_price + main_dish_base_price + extra ingredients price
    float expectedPrice =
        drinkOption1.getPrice()
            + mainDishOption2.getPrice()
            + ingredient3.getPrice() * 9
            + ingredient4.getPrice() * 2;
