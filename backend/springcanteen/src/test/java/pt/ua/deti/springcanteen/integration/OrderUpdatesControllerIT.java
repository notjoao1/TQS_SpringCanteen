...
class OrderUpdatesControllerIT {
    ...
    void whenReceiveUpdateOrder_FromIDLE_toPREPARING_thenSendUpdateThroughWebsockets() throws Exception {
        ...
        orderUpdateRequest.setOrderId(1L);
        ...
        // assert
        verify(orderServiceSpy, times(1)).changeOrderStatus(1L, OrderStatus.PREPARING);
        verify(orderNotifierServiceSpy, times(1)).sendOrderStatusUpdates(any(), any());
    }

    @Test
    void whenReceiveUpdateOrder_FromNOT_PAID_to_IDLE_thenSendNewOrderThroughWebsockets() throws Exception {
        // setup
        testOrder.setOrderStatus(OrderStatus.NOT_PAID);
        orderRepository.save(testOrder);
        OrderUpdateRequestDTO orderUpdateRequest = new OrderUpdateRequestDTO();
        orderUpdateRequest.setOrderId(1L);
        orderUpdateRequest.setNewOrderStatus(OrderStatus.IDLE);
        ...
        // assert
        verify(orderServiceSpy, times(1)).changeOrderStatus(1L, OrderStatus.IDLE);
        verify(orderNotifierServiceSpy, times(1)).sendNewOrder(any());
    }
