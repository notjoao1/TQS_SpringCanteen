            ...
            {priorityIdleOrders
              .map((order: CookOrder, index: number) => (
                <Box pt={2} key={order.id}>
                  <OrderCard index={index} order={order} isPriority={true} orderStatus={OrderStatus.IDLE} updateStatusMethod={sendOrderStatusUpdate}/>
                </Box>
              ))
            }
            {regularIdleOrders
              .map((order: CookOrder, index: number) => (
                <Box pt={2} key={order.id}>
                  <OrderCard index={index} order={order} isPriority={false} orderStatus={OrderStatus.IDLE} updateStatusMethod={sendOrderStatusUpdate}/>
                </Box>
              ))
            }
          </Paper>
        ...
        <Grid item xs={12} md={6}>
          ...
          >
            ...
            {priorityPreparingOrders
              .map((order: CookOrder, index: number) => (
                <Box pt={2} key={order.id}>
                  <OrderCard index={index} order={order} isPriority={true} orderStatus={OrderStatus.PREPARING} updateStatusMethod={sendOrderStatusUpdate}/>
                </Box>
              ))
            }
            {regularPreparingOrders
              .map((order: CookOrder, index: number) => (
                <Box pt={2} key={order.id}>
                  <OrderCard index={index} order={order} isPriority={false} orderStatus={OrderStatus.PREPARING} updateStatusMethod={sendOrderStatusUpdate}/>
                </Box>
              ))
            }
          </Paper>
