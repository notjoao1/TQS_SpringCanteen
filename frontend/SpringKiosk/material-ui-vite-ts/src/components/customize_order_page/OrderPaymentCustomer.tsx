...
const OrderPaymentCustomer = ({ onFormSubmit }: OrderPaymentCustomerProps) => {
  ...
  const [counter, setCounter] = useState(0); // in order to avoid validation on first render
  ...
  }, [nameOnCard, cardNumber, expirationDate, displayForm]);
  ...
  return (
    ...
    >
      <FormControl>
        ...
        >
          <FormControlLabel value={PaymentPlace.KIOSK} control={<Radio />} label={PaymentPlace.KIOSK} id="kiosk" onClick={() => setDisplayForm(true)} />
          <FormControlLabel value={PaymentPlace.DESK} control={<Radio />} label={PaymentPlace.DESK} id="desk" onClick={() => setDisplayForm(false)} />
        </RadioGroup>
      </FormControl>
      <Box
        pt={2}
